            ...
            (item) => (
              <td onDoubleClick={e => this.props.history.push("/admin/Package/" + item.Id + '/view')}>
                <CBadge color={getBadge(item.IsActived)}>
                  {item.IsActived ? 'Đang sử dụng' : 'Đã khóa'}
                </CBadge>
              </td>
            ),
          'Title':
    ...
    return (
      <CDropdown className="dropdown-sm">
        ...
        <CDropdownMenu>
          ...
          {hasPermissions(['ADMINPACKAGE_UPDATEPACKAGEAPI']) && <CDropdownItem to={"/admin/Package/" + item.Id}>Sửa</CDropdownItem>}
          {hasPermissions(['ADMINPACKAGE_CREATEORUPDATEPRICEPOLICYAPI']) && <CDropdownItem to={"/admin/price-policy/" + item.Id + '/form/new'}>Thiết lập giá</CDropdownItem>}
          {/* {hasPermissions(['ADMINPACKAGE_GETPACKAGEDETAILAPI']) && <CDropdownItem disabled>Copy gói</CDropdownItem>} */}
          {hasPermissions(['ADMINPACKAGE_DELETEPACKAGEAPI']) && <CDropdownItem divider />}
          {hasPermissions(['ADMINPACKAGE_DELETEPACKAGEAPI']) && <CDropdownItem disabled>Xóa</CDropdownItem>}
        </CDropdownMenu>
      </CDropdown>
    )
  }
          ...
          <CForm onSubmit={this.handleFilterSubmit.bind(this)}>
            <div className="d-flex flex-wrap">
              ...
              <div className="filter-item">
                ...
                <InputSelect name="Status" options={[{ value: '2', label: 'Tất cả'}, { value: '1', label: 'Đang sử dụng' }, { value: '0', label: 'Đã khóa' }]} defaultValue={query.Status} applyCallback={this.handleChange} />
              </div>
              <div className="filter-item">
              ...
              <div className="flex-grow-1">
                ...
                {hasPermissions(['ADMINPACKAGE_CREATEPACKAGEAPI']) && <CButton className="nowrap" color="success" to="/admin/Package/new"><VIcon size={'sm'} name='cilPlus' /> Thêm mới</CButton>}
                <CButton color="warning" to={'/admin/Package/?v=' + new Date().getTime()}>Xóa tìm kiếm</CButton>
                </div>
              </div>
            </div>
          </CForm>
